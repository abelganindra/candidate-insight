...
const LocationPage = () => {
  ...
  const entity = {
    ...
    fields: [
      ...
      { name: "locationaddress", label: "Address", multiline: true }, // multiline textarea
    ],
  };
  ...
  const handleSubmit = async (data) => {
    ...
    try {
      if (editing) {
        await api.put(`${entity.endpoint}/${editing.locationcode}`, data);
        showMessage(`Location ${editing.locationcode} updated successfully!`);
      } else {
        await api.post(entity.endpoint, data);
        showMessage("New location created successfully!");
      }
      setEditing(null);
  ...
  const handleDelete = async (row) => {
    if (!window.confirm(`Are you sure you want to delete location ${row.locationname} (${row.locationcode})?`))
      return;

    setIsLoading(true);
    try {
      await api.delete(`${entity.endpoint}/${row.locationcode}`);
      showMessage(`Location ${row.locationcode} deleted successfully.`);
      fetchLocations();
    } catch (err) {
      ...
      <div className="max-w-7xl mx-auto space-y-8">
        ...
        <header className="py-4 border-b border-gray-200">
          ...
          <p className="mt-2 text-lg text-gray-500">
            Create, manage, and track your business locations.
          </p>
        </header>
        ...
        <div className="mt-8">
            <EntityUpload
                entity={{
                    name: "location", // lowercase for filename validation
                    endpoint: "/locations",
                    fields: [
                    { name: "locationcode", label: "Location Code", required: true },
                    { name: "locationname", label: "Location Name", required: true },
